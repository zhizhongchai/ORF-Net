...
from .detection_utils import (annotations_to_instances, build_augmentation,
                              transform_instance_annotations, build_strong_augmentation)
...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        if self.basis_loss_on and self.is_train:
            semantic_path = dataset_dict["file_name"].replace('mask', 'mask_label')

            if os.path.exists(semantic_path) and 'mask' in semantic_path:
                basis_sem_gt = cv2.imread(semantic_path)[:, :, 0] / 255
            else:
                basis_sem_gt = np.zeros((dataset_dict['width'], dataset_dict['height']))
        ...
        if self.is_train:
            ...
            #     # print(239, file_name)
            #     # print(dataset_dict)
            #     dataset_dict['instances'].dots = dataset_dict['instances'].gt_classes - 1
            # dataset_dict['file_name']=file_name
            if dataset_dict['instances'].gt_classes.shape[0] > 0:
                dataset_dict['instances'].file_name = [file_name for i in
                                                       range(dataset_dict['instances'].gt_classes.shape[0])]

            semantic_box_path = dataset_dict["file_name"].replace('.png', '_box.png')
            if os.path.exists(semantic_box_path):
                box_sem_gt = cv2.imread(semantic_box_path)[:, :, 0] / 255
            else:
                if 'mask' not in file_name:
                    # print('not exists!', semantic_box_path)
                    box_sem_gt = np.zeros((dataset_dict['width'], dataset_dict['height']))
                else:
                    box_sem_gt = cv2.imread(dataset_dict["file_name"].replace('mask', 'mask_label'))[:, :, 0] / 255

            box_sem_gt = transforms.apply_segmentation(box_sem_gt)
            box_sem_gt = torch.as_tensor(box_sem_gt.astype("float"))
            # print(258, box_sem_gt.shape)
            dataset_dict["box_sem"] = box_sem_gt
