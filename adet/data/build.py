...
from adet.data.common import (
    AspectRatioGroupedOmniDatasetTwoCrop,
)
...
def build_omni_detection_train_loader_two_crops(cfg, mapper=None):
    # if cfg.DATASETS.CROSS_DATASET:  # cross-dataset (e.g., coco-additional)
    box_dicts = get_detection_dataset_dicts(
        cfg.DATASETS.TRAIN_BOX,
        filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
        min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
        if cfg.MODEL.KEYPOINT_ON
        else 0,
    ...
    dot_dicts = get_detection_dataset_dicts(
        cfg.DATASETS.TRAIN_DOT,
        filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
        min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
        if cfg.MODEL.KEYPOINT_ON
        else 0,
    ...
    mask_dicts = get_detection_dataset_dicts(
        cfg.DATASETS.TRAIN_MASK,
        filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
        min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
        if cfg.MODEL.KEYPOINT_ON
        else 0,
    ...
    box_dataset = DatasetFromList(box_dicts, copy=False)

    dot_dataset = DatasetFromList(dot_dicts, copy=False)

    mask_dataset = DatasetFromList(mask_dicts, copy=False)
    ...
    if mapper is None:
        mapper = DatasetMapper(cfg, True)
    box_dataset = MapDataset(box_dataset, mapper)
    dot_dataset = MapDataset(dot_dataset, mapper)
    mask_dataset = MapDataset(mask_dataset, mapper)
    ...
    logger.info("Using training sampler {}".format(sampler_name))
    if sampler_name == "TrainingSampler":
        box_sampler = TrainingSampler(len(box_dataset))
        dot_sampler = TrainingSampler(len(dot_dataset))
        mask_sampler = TrainingSampler(len(mask_dataset))
        unlabel_sampler = TrainingSampler(len(unlabel_dataset))
    elif sampler_name == "RepeatFactorTrainingSampler":
        raise NotImplementedError("{} not yet supported.".format(sampler_name))
    else:
        raise ValueError("Unknown training sampler: {}".format(sampler_name))
    return build_omni_batch_data_loader_two_crop(
        (box_dataset, dot_dataset, mask_dataset, unlabel_dataset),
        (box_sampler, dot_sampler, mask_sampler, unlabel_sampler),
        cfg.SOLVER.IMG_PER_BATCH_BOX,
        cfg.SOLVER.IMG_PER_BATCH_DOT,
        cfg.SOLVER.IMG_PER_BATCH_MASK,
...
# batch data loader
def build_omni_batch_data_loader_two_crop(
        dataset,
        sampler,
        total_batch_size_box,
        total_batch_size_dot,
        total_batch_size_mask,
    ...
    batch_size_box = total_batch_size_box // world_size
    batch_size_dot = total_batch_size_dot // world_size
    batch_size_mask = total_batch_size_mask // world_size
    batch_size_unlabel = total_batch_size_unlabel // world_size

    box_dataset, dot_dataset, mask_dataset, unlabel_dataset = dataset
    box_sampler, dot_sampler, mask_sampler, unlabel_sampler = sampler

    if aspect_ratio_grouping:
        box_data_loader = torch.utils.data.DataLoader(
            box_dataset,
            sampler=box_sampler,
        ...
        dot_data_loader = torch.utils.data.DataLoader(
            dot_dataset,
            sampler=dot_sampler,
            num_workers=num_workers,
            batch_sampler=None,
        ...
        mask_data_loader = torch.utils.data.DataLoader(
            mask_dataset,
        ...
        return AspectRatioGroupedOmniDatasetTwoCrop(
            (box_data_loader, dot_data_loader, mask_data_loader, unlabel_data_loader),
            (batch_size_box, batch_size_dot, batch_size_mask, batch_size_unlabel),
        )
    else:
