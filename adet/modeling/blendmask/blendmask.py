...
class BlendMask(nn.Module):
    ...
    def forward(self, batched_inputs, branch='supervised'):
        ...
        if self.training:
            return self.forward_train(images, features, batched_inputs)
        else:
            return self.forward_test(images, features, batched_inputs)

    def forward_train(self, images, features, batched_inputs):
        if self.combine_on:
        ...
        detector_results, detector_losses = self.blender(basis_out["bases"], proposals, gt_instances)
