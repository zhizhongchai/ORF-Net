...
class MaskBranch(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.mask_head = MaskHead(cfg, [input_shape[f] for f in self.in_features])
        self.in_channels_to_top_module = self.mask_head.in_channels_to_top_module
        self.num_classes = cfg.MODEL.FCOS.NUM_CLASSES
        self.mask_output = MaskOutput(cfg)
        self.cfg = cfg

    def forward(self, images, features, gt_instances=None, basis_sem=None, branch=''):
        """
        Arguments:
        ...
        logits_pred_list = self.mask_head(features)
        if self.training:
            basis_sem = basis_sem.float()
            gt_masks = []
            for im_i in range(len(gt_instances)):
                gt_masks.append(basis_sem)
            results, losses = self.mask_output.losses(
                    logits_pred_list, locations, gt_instances, gt_masks
                )
