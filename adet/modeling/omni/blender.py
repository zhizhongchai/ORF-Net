    ...
    def __call__(self, bases, proposals, gt_instances):
        if gt_instances is not None:
            ...
            if pos_inds.numel() == 0:
                ...
                #         box.pred_masks = proposals.pred_classes.view(
                #             -1, 1, self.pooler_resolution, self.pooler_resolution)
                #     return proposals, {"loss_mask": sum([x.sum() * 0 for x in attns]) + bases[0].sum() * 0}
                ...
                pred_mask_logits = pred_mask_logits.view(
                    -1, 1, self.pooler_resolution, self.pooler_resolution)
                return pred_mask_logits, {"loss_mask": sum([x.sum() * 0 for x in attns]) + bases[0].sum() * 0}
            ...
            return pred_mask_logits, {"loss_mask": mask_loss}
        else:
            # no proposals
            total_instances = sum([len(x) for x in proposals])
            if total_instances == 0:
