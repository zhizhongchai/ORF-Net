...
class DotBranch(nn.Module):
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.dot_head = DotHead(cfg, input_shape_head)

    def forward(self, images, features_dict, gt_instances):
        features = [features_dict[f] for f in self.in_features]
        clss_per_level, agn_hm_pred_per_level, cls_towers = self.dot_head(features)

        grids = self.compute_grids(features)
        shapes_per_level = grids[0].new_tensor([(x.shape[2], x.shape[3]) for x in clss_per_level])

        if not self.training:
            # proposals, losses = self.inference(
            #     images, clss_per_level, reg_pred_per_level,
            #     agn_hm_pred_per_level, grids)
            return clss_per_level
        else:
            ...
            logits_pred, agn_hm_pred = self._flatten_outputs(clss_per_level, agn_hm_pred_per_level)
            ...
            losses = self.losses(pos_inds, flattened_hms, logits_pred)
            # losses['train_info']['logits_pred_pred'] = clss_per_level

            losses['train_info']['ori_logits_pred'] = clss_per_level
            losses['train_info']['logits_pred'] = logits_pred
            return losses

    def losses(self, pos_inds, flattened_hms, logits_pred):
        '''
        Inputs:
            pos_inds: N
            labels: N
        ...
        losses = dict()
        if self.cfg.MODEL.OMNI.DOT_WEIGHT_ON:
            train_info = dict()
            train_info['flattened_hms'] = flattened_hms
            train_info['pos_inds'] = pos_inds
            train_info['logits_pred'] = logits_pred
            train_info['num_pos_avg'] = num_pos_avg
            losses['train_info'] = train_info
            return losses
        if not self.only_proposal:
            pos_loss, neg_loss = binary_heatmap_focal_loss_jit(
                logits_pred, flattened_hms, pos_inds,
    ...
    def _get_ground_truth(self, grids, shapes_per_level, gt_instances):
        ...
        for i in range(len(gt_instances)):  # images
            ...
            area = torch.ones_like(area) * 1600
