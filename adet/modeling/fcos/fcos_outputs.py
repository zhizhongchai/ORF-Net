...
class FCOSOutputs(nn.Module):
    ...
    def compute_pseudo_targets_for_locations(self, locations, results, logits_pred):
        num_loc_list = [len(loc) for loc in locations]
        ...
        size_ranges = torch.cat(loc_to_size_range, dim=0)
        locations = torch.cat(locations, dim=0)
        ...
        for im_i in range(len(results)):
            targets_per_im = results[im_i]
            bboxes = targets_per_im.pred_boxes.tensor
            labels_per_im = targets_per_im.pred_classes
            # no gt
            if bboxes.numel() == 0:
            ...
            area = targets_per_im.pred_boxes.area()
        ...
        pseudo_labels = pseudo_labels.flatten()

        pseudo_pos_inds = torch.nonzero(pseudo_labels != self.num_classes).squeeze(1)

        pseudo_num_pos_local = torch.ones_like(pseudo_pos_inds).sum()
        pseudo_num_pos_avg = max(reduce_mean(pseudo_num_pos_local).item(), 1.0)

        # prepare one_hot
        pseudo_class_target = torch.zeros_like(logits_pred)
        pseudo_class_target[pseudo_pos_inds, pseudo_labels[pseudo_pos_inds]] = 1

        return pseudo_class_target, pseudo_num_pos_avg
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges, num_loc_list):
        ...
        for im_i in range(len(targets)):
            ...
            if bboxes.shape[0] > 1:
                ...
                for single_box in bboxes:
                    if self.center_sample:
                        is_in_boxes_single = self.get_sample_region(
                            torch.unsqueeze(single_box, 0), self.strides, num_loc_list, xs,
                            ys, bitmasks=None, radius=self.radius
                        )
                    else:
                ...
                is_in_boxes_list.append(is_in_boxes * torch.unsqueeze((labels_per_im == 0), -1))
                # is_in_boxes_list.append(is_in_boxes)
        ...
        return {
            ...
            # "is_in_pred_boxes": is_in_pred_boxes_list,
        }

    def losses(self, logits_pred, logits_dot, reg_pred, locations, gt_instances, top_feats=None,
               ):
        """
        ...
        instances.logits_dot = cat([
            # Reshape: (N, C, Hi, Wi) -> (N, Hi, Wi, C) -> (N*Hi*Wi, C)
            x.permute(0, 2, 3, 1).reshape(-1, self.num_classes) for x in logits_dot
        ], dim=0, )
        ...
        return self.fcos_losses(instances, training_targets)

    def fcos_losses(self, instances, training_targets):
        losses, extras = {}, {}
        train_info = dict()
        ...
        # train_info['logits_ctrness'] = instances.ctrness_pred
        ...
        if self.box_quality == "ctrness":
            ...
            train_info['loss_reg_loc'] = self.loc_loss_func(ious, gious, None, 'none')

            train_info['reg_targets'] = instances.reg_targets
            train_info['loss_denorm'] = loss_denorm
            train_info['pos_inds'] = pos_inds
            # train_info['ctrness_targets'] = ctrness_targets
        ...
        losses["loss_box_cls"] = class_loss * self.loss_weight_cls
        ...
        if self.cfg.MODEL.OMNI.BOX_WEIGHT_ON:
            ...
            pos_loc_map = torch.unsqueeze(torch.sum(is_in_boxes, dim=-1), dim=-1)
            IoUMap = torch.zeros_like(pos_loc_map).float()
            IoUMap[pos_loc_map > 0] = ious

            train_info['IoUMap'] = IoUMap
    ...
    def predict_proposals(
            self, logits_pred, reg_pred, logits_uncat,
        ...
        x1 = logits_uncat[0].shape[-1] * logits_uncat[0].shape[-2]
        x2 = x1 + logits_uncat[1].shape[-1] * logits_uncat[1].shape[-2]
        x3 = x2 + logits_uncat[2].shape[-1] * logits_uncat[2].shape[-2]
        x4 = x3 + logits_uncat[3].shape[-1] * logits_uncat[3].shape[-2]
        x5 = x4 + logits_uncat[4].shape[-1] * logits_uncat[4].shape[-2]

        logits_pred = [logits_pred[0:x1], logits_pred[x1:x2], logits_pred[x2:x3],
                       logits_pred[x3:x4], logits_pred[x4:x5]]
    ...
    def forward_for_single_feature_map(
            self, locations, logits_pred, reg_pred, image_sizes, top_feat=None
    ):

        N, _, H, W = reg_pred.shape
        ...
        logits_pred = torch.unsqueeze(logits_pred, dim=0)
