...
class TOODOutputs(nn.Module):
    def __init__(self, cfg):
        super(TOODOutputs, self).__init__()
    ...
    def _get_ground_truth(self, locations, gt_instances, logits_pred, reg_pred, logits_dots, logits_masks):
        ...
        training_targets = self.compute_targets_for_locations(
            locations, gt_instances, loc_to_size_range, num_loc_list, logits_pred, reg_pred, logits_dots, logits_masks
        )
    ...
    def get_sample_region(self, boxes, strides, num_loc_list, loc_xs, loc_ys, bitmasks=None, radius=1):
        ...
        # print(161, center_x.shape, center_y.shape)
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges, num_loc_list, logits_pred, reg_pred,
                                      logits_dots, logits_masks):
        labels = []
        reg_targets = []
        target_inds = []
        xs, ys = locations[:, 0], locations[:, 1]
        # num_loc_list 0 16384 1 4096 2 1024 3 256 4 64
        ...
        for im_i in range(len(targets)):
            ...
            num_gt, num_bboxes = labels_per_im.size(0), locations.size(0)
            ...
            box_scores = cat([
                x[im_i].permute(1, 2, 0).reshape(-1, self.num_classes) for x in logits_pred], dim=0).sigmoid()
            dot_scores = cat([
                x[im_i].permute(1, 2, 0).reshape(-1, self.num_classes) for x in logits_dots], dim=0).sigmoid()
            mask_scores = cat([
                x[im_i].permute(1, 2, 0).reshape(-1, self.num_classes) for x in logits_masks], dim=0)
            scores = box_scores * dot_scores * mask_scores
            alignment_metrics = scores[:, labels_per_im].detach()

            # select top-k bbox as candidates for each gt
            _, candidate_idxs = alignment_metrics.topk(13, dim=0, largest=True)
            candidate_metrics = alignment_metrics[candidate_idxs, torch.arange(num_gt)]
            # print(246, candidate_metrics)
            ...
            # print(264, is_in_boxes.shape)
            pick_index = torch.zeros_like(alignment_metrics)
            pick_index[candidate_idxs] = 1.
            print(267, torch.sum(is_in_boxes))
            is_in_boxes = is_in_boxes * pick_index
            print(269, torch.sum(is_in_boxes))
            ...
            # print(261, labels_per_im, labels_per_im.shape)
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, gt_instances, top_feats=None, logits_dots=None,
               logits_masks=None):
        """
        ...
        training_targets = self._get_ground_truth(locations, gt_instances, logits_pred, reg_pred, logits_dots,
                                                  logits_masks)
    ...
    def fcos_losses(self, instances):
        ...
        if self.cfg.MODEL.FCOS.DA:
            class_loss = focal_loss_wo_sigmoid_jit(
...
focal_loss_wo_sigmoid_jit = torch.jit.script(focal_loss_wo_sigmoid)
